...
export function UptBerceritaDetailPage() {
  const { slug } = useParams<{ slug: string }>();
  const item = uptStories.find((s) => s.slug === slug);
  const related = uptStories.filter((s) => s.slug !== slug).slice(0, 3);

  usePageMeta(item?.title ?? "UPT Bercerita", item?.excerpt);

  if (!item) return <NotFoundPage />;

  return (
    <>
      <Breadcrumbs crumbs={[{ label: "Informasi", href: "/informasi" }, { label: "UPT Bercerita", href: "/informasi/upt-bercerita" }, { label: item.title }]} />
      ...
      {related.length > 0 && (
        <section className="border-t border-paper-200 bg-paper-100/50 py-12 sm:py-16">
          <Container>
            <h2 className="mb-6 font-display text-2xl text-ink-950">Cerita UPT Lainnya</h2>
            <div className="grid gap-5 sm:grid-cols-3">
              {related.map((r) => (
                <Link
                  key={r.slug}
                  to={`/informasi/upt-bercerita/${r.slug}`}
