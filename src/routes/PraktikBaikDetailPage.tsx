...
import { praktikBaikItems } from "@/content/berita";
import { usePageMeta } from "@/hooks/usePageMeta";
import { formatDate } from "@/lib/utils";
import { NotFoundPage } from "./NotFoundPage";

export function PraktikBaikDetailPage() {
  const { slug } = useParams<{ slug: string }>();
  const item = praktikBaikItems.find((b) => b.slug === slug);
  const related = praktikBaikItems.filter((b) => b.slug !== slug).slice(0, 3);

  usePageMeta(item?.title ?? "Praktik Baik", item?.excerpt);

  if (!item) return <NotFoundPage />;

  return (
    <>
      <Breadcrumbs crumbs={[{ label: "Informasi", href: "/informasi" }, { label: "Praktik Baik", href: "/informasi/praktik-baik" }, { label: item.title }]} />
      ...
      {related.length > 0 && (
        <section className="border-t border-paper-200 bg-paper-100/50 py-12 sm:py-16">
          <Container>
            <h2 className="mb-6 font-display text-2xl text-ink-950">Praktik Baik Lainnya</h2>
            <div className="grid gap-5 sm:grid-cols-3">
              {related.map((r) => (
                <Link
                  key={r.slug}
                  to={`/informasi/praktik-baik/${r.slug}`}
